...
class SchedulerDataSource extends ChangeNotifier
  ...
  List<Appointment> get visibleAppointments {
    ...
    var result = appointments
        .where((a) => visibleDateRange.inRangeOrBetween(a.startDate, a.endDate))
        .toList();
  ...
  List<AppointmentItem> get visibleAppointmentItems {
    if (visibleDateRange.isEmpty) {
      return visibleAppointments.fold<List<AppointmentItem>>([],
    ...
    return visibleAppointments.fold<List<AppointmentItem>>([],
        (appointmentItems, appointment) {
      appointmentItems.addAll(appointment.appointmentItems.where((a) =>
          visibleDateRange.inRange(a.startDate) ||
  ...
  List<AppointmentItem> visibleAppointmentItemsByDateRange(
    ...
    return getVisibleAppointmentItemsFrom((appointment) => appointmentService
        .getAppointmentItemsByDateRange(appointment, first, last));
  }

  List<AppointmentItem> get visibleAppointmentItemsByDay {
    var result = getVisibleAppointmentItemsFrom(
  ...
  List<AppointmentItem> getVisibleAppointmentItemsFrom(
    ...
    return visibleAppointments.fold<List<AppointmentItem>>([],
        (appointmentItems, appointment) {
      appointmentItems.addAll(getItems(appointment).where((a) =>
          visibleDateRange.inRange(a.startDate) ||
          visibleDateRange.inRange(a.endDate)));
  ...
  addAppointment(DateTime startDate, Duration duration, String subject,
    ...
    if (isAllDay) {
      appointment = Appointment(startDate.toLocalTime.startOfDay,
          ...
          color: color, isAllDay: isAllDay, recurrenceRule: recurrenceRule);
    } else {
      appointment = Appointment(
